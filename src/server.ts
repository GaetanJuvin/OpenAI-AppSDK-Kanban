...
const taskStore: Task[] = [...initialTasks];

function normalizeTaskInput(input: NewTaskInput): Task {
...
function upsertTask(task: Task): Task {
  const existingIndex = taskStore.findIndex(
...
function loadKanbanBoard(): KanbanBoard {
  const tasksById = Object.fromEntries(
    taskStore.map((task) => [task.id, task] as const),
  );
...
function buildWidgetState(board: KanbanBoard) {
  return {
...
function buildToolResponse(params: {
  ...
  const structuredContent = formatStructuredContent(
    params.board,
    params.statusFilter,
  );
...
function buildComponentHtml(cssUri: string, jsUri: string): string {
    ...
    TEMPLATE_URI,
    {
      ...
      _meta: {
        ...
        "openai/widgetDescription":
          "Renders a kanban layout that groups tasks by status.",
        "openai/widgetCSP": {
          connect_domains: [],
          resource_domains: [],
        },
